...
- `Ctrl-H` / `Ctrl-J` / `Ctrl-K` / `Ctrl-L`: send the key to the attached remote tmux client without the prefix, for root-table bindings such as pane navigation. If tmux has no binding for the key, the application in the pane receives it.
- `Ctrl-A` then `x`: send prefixed `x`, then wait for an answer to tmux's kill-pane confirmation. `y`, `n`, or `Esc` goes to tmux and restores normal prompt controls. Other keys ring the bell and leave the draft unchanged.
- `q` / `v` / `h` / `j` / `k` / `l` / `b` / `w` / `e` / `0` / `$` / `Enter` / `Ctrl-U` / `Ctrl-D` / `PageUp` / `PageDown` in scrollback: send the unprefixed key to tmux. Only `q` exits copy mode and restores normal prompt controls.
...
Attach mode establishes an SSH control connection during discovery. Every submission reuses it, which avoids repeated SSH key exchange and authentication. While the inline prompt is open, it also keeps one extra channel on this connection running a remote POSIX shell. Pane and window shortcuts are written to that shell as single lines, so each key costs about half a network round trip, not a new SSH session, a remote login shell, and a local prompt restart. The channel does not prompt for authentication. If it cannot start, for example because the control connection closed and SSH needs a password, pending shortcuts fall back to a normal SSH command in order, with any prompt shown in the controller terminal. The private control socket uses a short directory under `/tmp` to stay below Unix-socket path limits on macOS. The control socket and temporary files are removed when the controller exits.
